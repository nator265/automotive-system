...
const HiresPage = () => {
  ...
  const [showHiresDropdown, setShowHiresDropdown] = useState(false);
  ...
  const calculateMetrics = (month: string) => {
    ...
    const monthHires = hireBookings.filter((b: Booking) => {
      const bookingDate = new Date(b.date);
      return bookingDate >= startOfMonth && bookingDate <= endOfMonth && b.status === "accepted";
    });

    const totalRevenue = monthHires.reduce((sum: number, b: Booking) => sum + getPayment(b.carId), 0);

    return { hires: monthHires.length, revenue: totalRevenue };
  };
  ...
  const calculatePeriodMetrics = (days: number) => {
    ...
    const recentHires = hireBookings.filter((b: Booking) => {
      const bookingDate = new Date(b.date);
      return bookingDate >= periodAgo && b.status === "accepted";
    });

    const totalRevenue = recentHires.reduce((sum: number, b: Booking) => sum + getPayment(b.carId), 0);
  ...
  const filteredHireBookings = hireBookings.filter((b: Booking) =>
  ...
  return (
    <main className="w-full transition-all duration-300 mt-5 p-4 md:p-6">
      <div className="mb-6 pb-4 border-b border-gray-200 flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-gray-800">Hires</h1>
          <p className="text-gray-500 text-sm">Manage car hire bookings</p>
        </div>
        <button
          onClick={() => setShowAddCarModal(true)}
          className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 flex items-center gap-2"
        >
          <span>+</span> Add Car for Hire
        </button>
      </div>
        ...
        <div className="bg-blue-50 p-4 rounded-lg shadow relative">
          ...
          <div className="mt-2 flex items-center gap-2">
            ...
            <div className="relative">
              <button
                onClick={() => setShowHiresDropdown(!showHiresDropdown)}
      ...
      <div className="bg-white rounded-lg shadow p-4 mb-10">
        ...
        <div className="flex space-x-1 mb-4 border-b">
          ...
          >
            Pending ({hireBookings.filter((b) => b.status === "pending").length}
            )
          </button>
          <button
        ...
        {/* Table */}
        {(() => {
          const filteredBookings = hireBookings.filter((booking: Booking) => {
        ...
        {filteredHireBookings.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              ...
              <tbody>
                {filteredHireBookings.map((booking) => (
                  <tr key={booking.id} className="border-t">
                    <td className="py-3 text-sm text-gray-700">
                      {booking.name}
                    </td>
                    ...
                    <td className="py-3 text-sm text-gray-700">
                      ₦{getPayment(booking.carId).toLocaleString()}
                    </td>
                    <td className="py-3 text-sm text-gray-700">
                      {booking.date}
                    </td>
      ...
      {showAddCarModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-md">
            <h3 className="text-lg font-semibold mb-4">Add Car for Hire</h3>
